...
export type DashboardShellProps = {
  ...
  /** When true (mobile link detail), main list is hidden so the side panel can use full height. */
  hideMain?: boolean;
  main: ReactNode;
  side: ReactNode;
};
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row">
          ...
          {!sideOpen && (
            <button
              ...
              className={cn(
                ...
                "fixed left-1/2 flex w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 items-center justify-center py-3",
      ...
      {sideOpen ? (
        <DashboardSideSlot
          className={cn(
            hideMain && "min-h-0 flex-1 border-t-0 md:min-h-0 md:flex-none md:border-l md:border-t-0",
          )}
        >
          {side}
        </DashboardSideSlot>
